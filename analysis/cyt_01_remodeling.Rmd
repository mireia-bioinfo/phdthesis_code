...
## 3) Defining Induced Regulatory Elements
...
ols <- findOverlaps(at, k27, maxgap=1)

re <- at[queryHits(ols),]
colnames(mcols(re)) <- paste0("atac.", colnames(mcols(re)))
mcols(k27) <- mcols(k27)[,c(1,3,8)]
...
# save(re, file="../data/CYT/REs/REs_endoc_granges.rda")
```

```{r plot-cor-res, fig.width=4}
...
```{r plot-cons, fig.height=4, fig.width=4}
