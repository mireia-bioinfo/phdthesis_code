...
## Calculate number of reads in binned genome
load("~/data/IRB_database/shared/hg19_len.rda")
hg19 <- GRanges(seqnames = len$chr,
                ranges = IRanges(start=1, end=len$length))

bins <- unlist(tile(hg19, width=10e3))
saf <- data.frame(bins)[,1:3]
colnames(saf) <- c("Chr", "Start", "End")
saf$Strand <- "+"
saf$GeneID <- paste0("genome_", 1:nrow(saf))

bams <- list.files("../data/ATAC/BAMs",
                   pattern="*.bam$",
                   full.names=TRUE)
bams <- bams[!grepl("_2.bam", bams)]

names <- gsub(".bam", "", basename(bams))

counts <- Rsubread::featureCounts(bams,
                                  annot.ext=saf,
                                  nthreads=8)
colnames(counts$counts) <- names
save(counts, file="data/IFNa/ATAC/QC/ATAC_10kb_correlation_rsubread.rda")
